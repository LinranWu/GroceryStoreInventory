...
#include <locale.h>
#include <monetary.h>
/*This function determine and print total wholesale cost for all grocery items which is calculated as the sum of (wholesale price * wholesale quantity) for all grocery items*/
void option2(Node* trav){
	float sum;
	char local[23];	/*this is the string to contain the monetary value*/

	sum = 0;
	while(trav != NULL){	/*go through the list*/
		sum += ((trav->grocery_item.pricing.wholesalePrice)*(trav->grocery_item.pricing.wholesaleQuantity));
		trav = trav->next;
	}
	setlocale(LC_ALL, "");	/*set format*/
	strfmon(local, 22, "%n", sum);	/*set as monetary value*/
	printf("Total Wholesale cost:%s\n",local);
}
